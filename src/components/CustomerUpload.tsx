...
type CustomerWizardCopy = SharedContent['customerWizard']
...
function ProgressIndicator({ current, t }: { current: number; t: CustomerWizardCopy }) {
  return (
    <div className="flex flex-col items-center gap-3 mb-10">
      <p className="text-[10px] uppercase tracking-[0.3em] text-memorial-ink-soft">
        {t.progress(current, TOTAL_STEPS)}
      </p>
...
function Step1({
  ...
  return (
    <StepWrapper>
      <h2 className="font-display text-3xl text-memorial-ink mb-2">{t.title}</h2>
      <p className="text-memorial-ink-soft text-[15px] mb-8">{t.subtitle}</p>

      <div className="grid sm:grid-cols-2 gap-4 mb-8">
        {([
          { type: 'human' as const, icon: <Heart className="w-8 h-8" strokeWidth={1.3} />, title: t.optionHuman, accent: 'hover:bg-memorial-bronze/10 data-[selected=true]:bg-memorial-bronze/10 data-[selected=true]:border-memorial-bronze-deep' },
          { type: 'pet' as const, icon: <PawPrint className="w-8 h-8" strokeWidth={1.3} />, title: t.optionPet, accent: 'hover:bg-memorial-sage/10 data-[selected=true]:bg-memorial-sage/10 data-[selected=true]:border-memorial-sage-deep' },
        ]).map((c) => (
          <button
            key={c.type}
      ...
      <div className="space-y-6">
        <div>
          ...
          <input
            type="text"
            placeholder={isHuman || !data.subjectType ? t.namePlaceholderHuman : t.namePlaceholderPet}
        ...
        <div className="grid sm:grid-cols-2 gap-6">
          ...
          <div>
            <label className="text-[11px] uppercase tracking-widest text-memorial-ink-soft block mb-2">
              {isHuman ? t.deathDateHuman : t.deathDatePet}
            </label>
...
function Step2({
  ...
  const styles: { id: AlbumStyle; label: string; desc: string }[] = [
    ...
    { id: 'classic', label: t.classicLabel, desc: t.classicDesc },
    { id: 'timeless', label: t.timelessLabel, desc: t.timelessDesc },
  ]

  return (
    <StepWrapper>
      <h2 className="font-display text-3xl text-memorial-ink mb-2">{t.title}</h2>
      <p className="text-memorial-ink-soft text-[15px] mb-10">{t.subtitle}</p>
...
function Step3({
  ...
  toastsCopy: CustomerWizardCopy['toasts']
}) {
  ...
  const handleFiles = async (files: FileList | null) => {
    ...
    if (remaining <= 0) { toast.error(toastsCopy.maxReached(MAX_MEDIA)); return }
  ...
  return (
    <StepWrapper>
      ...
      <div className="flex items-baseline justify-between mb-2">
        <h2 className="font-display text-3xl text-memorial-ink">{t.title}</h2>
        <span className={cn('text-sm font-medium tabular-nums', countColor)}>{t.countLabel(count, MAX_MEDIA)}</span>
      </div>
      <p className="text-memorial-ink-soft text-[15px] mb-2">
        {t.intro(MIN_MEDIA, MAX_MEDIA)}
      </p>
      {count > 0 && count < MIN_MEDIA && (
        <p className="text-[12px] text-amber-600 mb-4">{t.moreNeeded(MIN_MEDIA - count, MIN_MEDIA)}</p>
      )}
      ...
      >
        {isCompressing ? (
          <><Loader2 className="w-8 h-8 text-memorial-bronze-deep animate-spin" /><p className="text-memorial-ink-soft text-[14px]">{t.compressing}</p></>
        ) : (
          <>
            <Upload className="w-8 h-8 text-memorial-bronze-deep" strokeWidth={1.3} />
            <p className="font-display text-xl text-memorial-ink">{t.dropZoneTitle}</p>
            <p className="text-[13px] text-memorial-ink-soft">{t.dropZoneOr} <span className="text-memorial-bronze-deep underline underline-offset-2">{t.dropZoneSelect}</span></p>
            <p className="text-[11px] text-memorial-ink-soft/60 mt-1">{t.dropZoneRange(MIN_MEDIA, MAX_MEDIA)}</p>
          </>
        )}
      ...
      <div className="border-t border-memorial-line pt-6">
        <p className="text-[11px] uppercase tracking-widest text-memorial-ink-soft mb-4">{t.musicTitle}</p>
        ...
        <div className="memorial-card rounded-xl p-4 border border-memorial-line flex gap-2 items-center">
          <input
            type="url"
            placeholder={t.pixabayPlaceholder}
            value={pixabayLink}
            onChange={(e) => {
              setPixabayLink(e.target.value)
              onMusicChange(e.target.value.trim() ? { type: 'pixabay', value: e.target.value.trim(), label: t.pixabayLabel } : { type: 'none', value: '', label: t.noSelection })
            }}
          ...
          <a href="https://pixabay.com/music/" target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-[12px] text-memorial-bronze-deep hover:underline whitespace-nowrap">
            {t.pixabayOpen} <ExternalLink className="w-3 h-3" />
          </a>
        </div>
      </div>
    </StepWrapper>
...
function Step4({
  ...
  const styleLabels: Record<AlbumStyle, string> = {
    modern: styleNamesCopy.modern,
    classic: styleNamesCopy.classic,
    timeless: styleNamesCopy.timeless,
  }
  const shippingLabels: Record<ShippingZone, string> = {
    CH: shippingZonesCopy.ch,
    EU: shippingZonesCopy.eu,
    WORLD: shippingZonesCopy.world,
  }
  ...
  return (
    <StepWrapper>
      ...
      {/* Contact */}
      <p className="text-[11px] uppercase tracking-[0.25em] text-memorial-ink-soft mb-4">{t.contactSection}</p>
      <div className="space-y-6 mb-10">
        <div>
          <label className="text-[11px] uppercase tracking-widest text-memorial-ink-soft block mb-2">{t.nameLabel}</label>
          <input type="text" placeholder={t.namePlaceholder} value={data.contactName} onChange={setField('contactName')} className="memorial-underline-input w-full text-[15px] text-memorial-ink" />
        </div>
        <div className="grid sm:grid-cols-2 gap-6">
          <div>
            <label className="text-[11px] uppercase tracking-widest text-memorial-ink-soft block mb-2">{t.emailLabel}</label>
            <input type="email" placeholder={t.emailPlaceholder} value={data.contactEmail} onChange={setField('contactEmail')} className="memorial-underline-input w-full text-[15px] text-memorial-ink" />
          </div>
          <div>
            <label className="text-[11px] uppercase tracking-widest text-memorial-ink-soft block mb-2">{t.phoneLabel}</label>
            <input type="tel" placeholder={t.phonePlaceholder} value={data.contactPhone} onChange={setField('contactPhone')} className="memorial-underline-input w-full text-[15px] text-memorial-ink" />
          </div>
        </div>
      </div>
      ...
      <div className="flex items-baseline justify-between mb-4">
        <p className="text-[11px] uppercase tracking-[0.25em] text-memorial-ink-soft">{t.addressSection}</p>
        <span className="text-[11px] text-memorial-ink-soft">
          {t.shippingPrefix} <span className="text-memorial-bronze-deep font-medium">{shippingLabels[zone]} · CHF {formatPrice(shipping)}</span>
        </span>
      </div>
      <div className="space-y-6 mb-8">
        <div>
          <label className="text-[11px] uppercase tracking-widest text-memorial-ink-soft block mb-2">{t.streetLabel}</label>
          <input type="text" placeholder={t.streetPlaceholder} value={data.street} onChange={setField('street')} className="memorial-underline-input w-full text-[15px] text-memorial-ink" />
        </div>
        <div className="grid grid-cols-3 gap-4">
          <div className="col-span-1">
            <label className="text-[11px] uppercase tracking-widest text-memorial-ink-soft block mb-2">{t.postalLabel}</label>
          ...
          <div className="col-span-2">
            <label className="text-[11px] uppercase tracking-widest text-memorial-ink-soft block mb-2">{t.cityLabel}</label>
            <input type="text" autoComplete="address-level2" placeholder={t.cityPlaceholder} value={data.city} onChange={setField('city')} className="memorial-underline-input w-full text-[15px] text-memorial-ink" />
          </div>
        </div>
        <div>
          <label className="text-[11px] uppercase tracking-widest text-memorial-ink-soft block mb-2">{t.countryLabel}</label>
      ...
      <div className="memorial-card rounded-2xl p-6 mb-8">
        <p className="text-[11px] uppercase tracking-widest text-memorial-ink-soft mb-4">{t.summaryTitle}</p>
        <SummaryRow label={t.summaryFor} value={step1.subjectName || '—'} />
        <SummaryRow label={t.summaryStyle} value={albumStyle ? styleLabels[albumStyle] : '—'} />
      ...
      >
        {isSubmitting ? (
          ...
          t.payCta(`CHF ${formatPrice(total)}`)
        )}
      </button>
...
function SuccessScreen({ t }: { t: CustomerWizardCopy['success'] }) {
  return (
    <div className="flex flex-col items-center justify-center text-center py-20">
      ...
      <h2 className="font-display text-3xl text-memorial-ink mb-3">{t.title}</h2>
...
export default function CustomerUpload() {
  ...
  const t = sharedContent.customerWizard
  ...
  const next = () => {
    if (!canAdvance()) {
      const msgs: Record<number, string> = {
        1: t.validation.pickAudience,
        2: t.validation.pickStyle,
        3: t.validation.uploadMin(MIN_MEDIA),
      }
      toast.error(msgs[step] ?? t.validation.fillRequired)
      return
    }
  ...
  const handleAddMedia = async (files: FileList) => {
    ...
    if (valid.length > 0) toast.success(t.toasts.filesAdded(valid.length))
  }

  const validateContactAndAddress = (): boolean => {
    if (!step4.contactEmail.trim()) { toast.error(t.validation.emailRequired); return false }
    if (!step4.contactName.trim()) { toast.error(t.validation.nameRequired); return false }
  ...
  const handleTestSubmit = async () => {
    ...
    if (!step1.subjectType || !albumStyle) { toast.error(t.validation.completeAllSteps); return }
    if (mediaItems.length < MIN_MEDIA) { toast.error(t.validation.minMediaError(MIN_MEDIA)); return }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (checkoutError || !checkoutData?.url) {
        setIsSuccess(true)
        toast.warning(t.toasts.orderCreatedNoPayment)
        return
      }
  ...
  if (isSuccess) return <SuccessScreen t={t.success} />

  return (
    <div className="max-w-3xl mx-auto px-4 py-10">
      <ProgressIndicator current={step} t={t} />
      ...
      <div className="flex items-center justify-between mt-10 pt-6 border-t border-memorial-line">
        ...
        {step < TOTAL_STEPS && (
          <button onClick={next} className="memorial-cta memorial-cta-primary flex items-center gap-2 rounded-full px-8 py-3 text-sm font-medium">
            {step === 3 ? t.nav.toStep4 : t.nav.next}
