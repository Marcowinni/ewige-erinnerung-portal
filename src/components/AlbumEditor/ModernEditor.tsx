...
interface Props {
  ...
  onPreview: () => void
  onBack: () => void
  onSave: (payload: { pages: PageConfig[]; assignments: Record<SlotPath, string> }) => void
  onPublish: () => void
  saving: boolean
  publishing: boolean
  isPublished: boolean
  // Optional: update per-image focal point in parent (admin context)
...
// Build page metadata from pages array using page-based slot paths
function buildPageMeta(pages: ReturnType<typeof useModernLayout>['pages']): PageMeta[] {
  return pages.map((p, i) => {
    ...
    switch (p.type) {
      ...
      case 'split':
        return {
          index: i, type: 'split', title: `Seite ${i + 1} — Zwei Bilder`,
          slots: [
            { path: sp(0), label: '1', gridArea: 'a' },
            { path: sp(1), label: '2', gridArea: 'b' },
          ],
          grid: { cols: '1fr', rows: '1fr 1fr', areas: '"a" "b"' },
        }
      case 'bleed':
        return {
          index: i, type: 'bleed', title: `Seite ${i + 1} — Vollbild`,
          slots: [{ path: sp(0), label: 'Vollbild', gridArea: 'a' }],
          grid: { cols: '1fr', rows: '1fr', areas: '"a"' },
        }
      case 'mosaic':
        return {
          index: i, type: 'mosaic', title: `Seite ${i + 1} — Collage`,
          slots: [
            { path: sp(0), label: '1', gridArea: 'a' },
            { path: sp(1), label: '2', gridArea: 'b' },
          ],
          grid: { cols: '1fr', rows: '1.4fr 1fr', areas: '"a" "b"' },
        }
      case 'stack':
        return {
          index: i, type: 'stack', title: `Seite ${i + 1} — Polaroids`,
          slots: [
            { path: sp(0), label: 'Polaroid 1', gridArea: 'a' },
            { path: sp(1), label: 'Polaroid 2', gridArea: 'b' },
          ],
          grid: { cols: '1fr 1fr', rows: '1fr', areas: '"a b"' },
        }
      case 'story':
        return {
          index: i, type: 'story', title: `Seite ${i + 1} — Erzählung`,
          slots: [
            { path: sp(0), label: 'Hero', gridArea: 'a' },
      ...
      case 'quote-card':
        return {
          index: i, type: 'quote-card', title: `Seite ${i + 1} — Zitat`,
          slots: [{ path: sp(0), label: 'Foto', gridArea: 'a' }],
          grid: { cols: '1fr', rows: '1fr', areas: '"a"' },
        }
      case 'twin-portrait':
        return {
          index: i, type: 'twin-portrait', title: `Seite ${i + 1} — Zwei Portraits`,
          slots: [
            { path: sp(0), label: 'Portrait 1', gridArea: 'a' },
            { path: sp(1), label: 'Portrait 2', gridArea: 'b' },
          ],
          grid: { cols: '1fr', rows: '1fr 1fr', areas: '"a" "b"' },
        }
      case 'close':
...
const LAYOUT_TYPES: { type: PageType; label: string; slots: number }[] = [
  { type: 'bleed',         label: 'Vollbild',       slots: 1 },
  { type: 'quote-card',    label: 'Zitat',          slots: 1 },
  { type: 'split',         label: 'Zwei Bilder',    slots: 2 },
  { type: 'stack',         label: 'Polaroids',      slots: 2 },
  { type: 'story',         label: 'Erzählung',      slots: 2 },
  { type: 'twin-portrait', label: 'Zwei Portraits', slots: 2 },
]

function LayoutWireframe({ type }: { type: PageType }) {
  const base = 'bg-memorial-bronze/30 rounded-[2px]'
  ...
  switch (type) {
    ...
    case 'split':
      ...
      return (
        <div className="w-full h-full p-1 grid gap-0.5" style={{ gridTemplateRows: '1fr 1fr' }}>
          <div className={base} />
          <div className={base} />
        </div>
      )
    case 'stack':
      ...
      return (
        <div className="w-full h-full p-1 flex gap-0.5">
          <div className={cn(base, 'flex-1')} style={{ transform: 'rotate(-3deg)' }} />
          <div className={cn(base, 'flex-1')} style={{ transform: 'rotate(2deg)' }} />
        </div>
      )
    case 'story':
      // Hero top (tall) + one supporting image below
      return (
        <div className="w-full h-full p-1 grid gap-0.5" style={{ gridTemplateRows: '1.4fr 1fr' }}>
          <div className={cn(base, 'opacity-60')} />
          <div className={base} />
        </div>
      )
    case 'quote-card':
      // Photo left half + quote text right half (or top/bottom on very narrow)
      return (
        <div className="w-full h-full p-1 flex gap-0.5">
          <div className={cn(base, 'flex-1')} />
          <div className="flex-1 flex flex-col justify-center items-center gap-0.5 p-0.5">
            <div className="w-3 h-0.5 bg-memorial-bronze/40 rounded" />
            <div className="w-5 h-0.5 bg-memorial-bronze/30 rounded" />
            <div className="w-4 h-0.5 bg-memorial-bronze/20 rounded" />
          </div>
        </div>
      )
    case 'twin-portrait':
      // Two portraits stacked (mobile) / side by side (desktop)
      return (
        ...
        {open && (
          <>
            ...
            <motion.div
              ...
              className="absolute top-full mt-2 left-1/2 -translate-x-1/2 z-50 memorial-card rounded-2xl p-3 w-[300px] shadow-xl border border-memorial-line"
            >
                ...
                {LAYOUT_TYPES.map((layout) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="text-[9px] font-display-italic text-memorial-ink-soft leading-none text-center">
                        {layout.label}
                      </span>
                      <span className="text-[8px] text-memorial-ink-soft/60">
                        {layout.slots} {layout.slots === 1 ? 'Bild' : 'Bilder'}
                      </span>
                      {isActive && (
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    isActive ? 'ring-2 ring-memorial-bronze-deep shadow-[0_0_0_4px_hsl(var(--memorial-bronze)/0.15)]' : 'ring-memorial-line hover:ring-memorial-bronze/60 hover:-translate-y-0.5'
                  )}
                ...
                >
                  ...
                  {count > 0 && (
                    <div
                      className={cn(
                        ...
                        multi
                          ? 'bg-memorial-bronze-deep text-memorial-canvas'
                          : 'bg-memorial-sage-deep text-memorial-canvas'
                      )}
                    >
                      {count}×
                    </div>
                  ...
                  {count === 0 && (
                    <div className="absolute top-1.5 right-1.5 px-2 h-[22px] rounded-full flex items-center justify-center text-[9px] uppercase tracking-widest font-medium bg-amber-100 text-amber-800 border border-amber-300">
                      Neu
                    </div>
                  )}

                  {isActive && (
                    <div className="absolute bottom-1.5 right-1.5 w-5 h-5 rounded-full bg-memorial-bronze-deep flex items-center justify-center ring-2 ring-white">
                      <Check className="w-3 h-3 text-memorial-canvas" strokeWidth={3} />
                    </div>
                  )}
                </button>
              )
...
function PageCard({
  ...
  onChangeLayout: (pageIdx: number, type: PageType) => void
    ...
    <div className="memorial-card rounded-2xl p-5">
      <div className="flex items-baseline justify-between mb-4 flex-wrap gap-3">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          {canToggleText && (
            <button
              ...
              className={cn(
                ...
                showText
                  ? 'border-memorial-bronze-deep bg-memorial-bronze/10 text-memorial-bronze-deep'
                  : 'border-memorial-line text-memorial-ink-soft hover:border-memorial-bronze/50'
              )}
            >
              <span className={cn('w-1.5 h-1.5 rounded-full', showText ? 'bg-memorial-bronze-deep' : 'bg-memorial-ink-soft/30')} />
          ...
          <p className="text-[11px] text-memorial-ink-soft">{meta.slots.length} {meta.slots.length === 1 ? 'Bild' : 'Bilder'}</p>
        </div>
      </div>
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    active
                      ? 'border-memorial-bronze-deep bg-memorial-bronze/10 text-memorial-bronze-deep'
                      : 'border-memorial-line text-memorial-ink-soft hover:border-memorial-bronze/50 hover:text-memorial-ink'
                  )}
                >
                  {phrase}
                </button>
        ...
        {meta.slots.map((s) => {
          ...
          return (
            <div
              ...
              className="relative rounded-lg overflow-hidden ring-1 ring-memorial-line hover:ring-memorial-bronze hover:shadow-lg transition-all duration-200 group"
            >
              <button
...
export function ModernEditor({
  ...
  const { pages, assignments, assign, orderedImages, usedUrls, changePageLayout, togglePageText, editPageText } = useModernLayout(imageUrls, initialPages)
  ...
  // ESC closes fullscreen + lock scroll
  useEffect(() => {
  ...
  // Usage tracking — every image must be used at least once to publish
  ...
  const unusedImages = useMemo(
    () => imageItems.filter((img) => (usageCount.get(img.previewUrl) ?? 0) === 0),
    [imageItems, usageCount]
  )
  const multipleImages = useMemo(
    () => imageItems.filter((img) => (usageCount.get(img.previewUrl) ?? 0) > 1),
    [imageItems, usageCount]
  )
  ...
  return (
    <div className="h-full overflow-y-auto bg-memorial-canvas">
      ...
      <div className="sticky top-0 z-30 bg-memorial-canvas/95 backdrop-blur-xl border-b border-memorial-line px-6 py-3 flex items-center justify-between">
        <div>
          <p className="text-[10px] uppercase tracking-[0.3em] text-memorial-ink-soft">Album gestalten</p>
      ...
      {imageItems.length > 0 && (
        <div className={cn(
          ...
          unusedImages.length > 0
            ...
            : 'bg-memorial-sage/10 border-memorial-sage/30'
        )}>
          <div className="flex items-center justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-3">
              {unusedImages.length > 0 ? (
                ...
                <Check className="w-5 h-5 text-memorial-sage-deep flex-shrink-0" strokeWidth={2.5} />
              )}
              <div>
                <p className={cn('text-[13px] font-medium', unusedImages.length > 0 ? 'text-amber-900' : 'text-memorial-sage-deep')}>
                  {unusedImages.length > 0
            ...
            {unusedImages.length > 0 && (
              <div className="flex items-center gap-1.5 overflow-x-auto max-w-full">
                ...
                {unusedImages.slice(0, 8).map((img) => (
                  <div
                    key={img.id}
                    className="w-10 h-10 rounded-md overflow-hidden flex-shrink-0 ring-2 ring-amber-300"
                    title="Noch nicht verwendet"
                  >
            ...
            {multipleImages.length > 0 && unusedImages.length === 0 && (
              <div className="flex items-center gap-1.5 overflow-x-auto max-w-full">
                <span className="text-[10px] uppercase tracking-widest text-memorial-bronze-deep flex-shrink-0">Mehrfach:</span>
                {multipleImages.slice(0, 8).map((img) => {
                  const count = usageCount.get(img.previewUrl) ?? 0
                  return (
                    <div
                      key={img.id}
                      className="relative w-10 h-10 rounded-md overflow-hidden flex-shrink-0 ring-2 ring-memorial-bronze"
                      title={`${count}× verwendet`}
                    >
                      <img src={img.previewUrl} alt="" className="w-full h-full object-cover" />
                      <span className="absolute -top-1 -right-1 bg-memorial-bronze-deep text-memorial-canvas text-[10px] font-bold rounded-full min-w-[16px] h-[16px] flex items-center justify-center px-1">
                        {count}
                      </span>
                    </div>
                  )
