...
interface Props {
  ...
  initialPages?: ClassicPageConfig[]
  onPreview: () => void
  onBack: () => void
  onSave: (payload: { pages: ClassicPageConfig[]; assignments: Record<SlotPath, string> }) => void
  onPublish: () => void
  saving: boolean
  publishing: boolean
  isPublished: boolean
...
function buildPageMeta(pages: ReturnType<typeof useClassicLayout>['pages']): PageMeta[] {
  return pages.map((p, i) => {
    ...
    switch (p.type) {
      ...
      case 'herald':
        return {
          index: i, type: 'herald', title: `Seite ${i + 1} — Portrait + Bild`,
          slots: [
            { path: sp(0), label: 'Gross', gridArea: 'a' },
            { path: sp(1), label: '2', gridArea: 'b' },
          ],
      ...
      case 'polaroids':
        return {
          index: i, type: 'polaroids', title: `Seite ${i + 1} — Polaroids`,
          slots: [
      ...
      case 'strip':
        return {
          index: i, type: 'strip', title: `Seite ${i + 1} — Streifen`,
          slots: [
            { path: sp(0), label: '1', gridArea: 'a' },
            { path: sp(1), label: '2', gridArea: 'b' },
          ],
      ...
      case 'tape':
        return {
          index: i, type: 'tape', title: `Seite ${i + 1} — Klebeband`,
          slots: [
            { path: sp(0), label: '1', gridArea: 'a' },
            { path: sp(1), label: '2', gridArea: 'b' },
          ],
      ...
      case 'diagonal':
        return {
          index: i, type: 'diagonal', title: `Seite ${i + 1} — Diagonal`,
          slots: [
            { path: sp(0), label: '1', gridArea: 'a' },
            { path: sp(1), label: '2', gridArea: 'b' },
          ],
      ...
      case 'envelope-letter':
        return {
          index: i, type: 'envelope-letter', title: `Seite ${i + 1} — Briefumschlag`,
          slots: [{ path: sp(0), label: 'Foto', gridArea: 'a' }],
          grid: { cols: '1fr', rows: '1fr', areas: '"a"' },
        }
      case 'pinned':
        return {
          index: i, type: 'pinned', title: `Seite ${i + 1} — Reiszwecken`,
          slots: [
            { path: sp(0), label: 'Foto 1', gridArea: 'a' },
            { path: sp(1), label: 'Foto 2', gridArea: 'b' },
          ],
          grid: { cols: '1fr', rows: '1fr 1fr', areas: '"a" "b"' },
        }
      case 'close':
...
// ── Layout types available in picker ─────────────────────────────────────────

const LAYOUT_TYPES: { type: ClassicPageType; label: string; slots: number }[] = [
  { type: 'bleed',            label: 'Vollbild',        slots: 1 },
  { type: 'envelope-letter',  label: 'Briefumschlag',   slots: 1 },
  { type: 'duo',              label: 'Zwei Bilder',     slots: 2 },
  { type: 'polaroids',        label: 'Polaroids',       slots: 2 },
  { type: 'tape',             label: 'Klebeband',       slots: 2 },
  { type: 'pinned',           label: 'Reiszwecken',     slots: 2 },
  { type: 'herald',           label: 'Portrait + Bild', slots: 2 },
  { type: 'strip',            label: 'Streifen',        slots: 2 },
  { type: 'diagonal',         label: 'Diagonal',        slots: 2 },
]

function LayoutWireframe({ type }: { type: ClassicPageType }) {
  ...
  switch (type) {
    ...
    case 'polaroids':
      // Two polaroids side by side, slightly rotated
      return (
    ...
    case 'tape':
      // Two photos with tape aesthetic, side by side
      return (
        <div className="w-full h-full p-1 flex gap-0.5">
          <div className={cn(base, 'flex-1')} style={{ transform: 'rotate(-4deg)' }} />
          <div className={cn(base, 'flex-1')} style={{ transform: 'rotate(3deg)' }} />
        </div>
      )
    case 'herald':
      // Large top + smaller below
      return (
        <div className="w-full h-full p-1 grid gap-0.5" style={{ gridTemplateRows: '1.4fr 1fr' }}>
          <div className={cn(base, 'opacity-70')} />
          <div className={base} />
        </div>
      )
    case 'strip':
      // Two photos side by side (horizontal strip)
      return (
        <div className="w-full h-full p-1 grid gap-0.5" style={{ gridTemplateColumns: '1fr 1fr' }}>
          <div className={base} />
          <div className={base} />
        </div>
      )
    case 'diagonal':
      // Two photos stacked, slightly offset
      return (
    ...
    case 'envelope-letter':
      // Photo in envelope frame
      return (
        <div className="w-full h-full p-1 flex flex-col gap-0.5">
          <div className="w-full h-2 bg-memorial-bronze/20 rounded-[1px]" style={{ clipPath: 'polygon(0 0, 100% 0, 50% 100%)' }} />
          <div className={cn(base, 'flex-1')} />
        </div>
      )
    case 'pinned':
      // Two photos with slight rotations, overlap hint
      return (
        <div className="w-full h-full p-1 flex flex-col gap-1 items-center">
          <div className={cn(base, 'w-[85%] flex-1')} style={{ transform: 'rotate(-3deg)' }} />
          <div className={cn(base, 'w-[85%] flex-1')} style={{ transform: 'rotate(2.5deg)' }} />
        </div>
      )
    default:
...
function LayoutPickerMini({
  ...
  currentType: ClassicPageType
  onChange: (type: ClassicPageType) => void
}) {
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    isActive
                      ? 'ring-2 ring-memorial-bronze-deep shadow-[0_0_0_4px_hsl(var(--memorial-bronze)/0.15)]'
                      : 'ring-memorial-line hover:ring-memorial-bronze/60 hover:-translate-y-0.5'
                  )}
                ...
                >
                  ...
                  {count > 0 && (
                    <div className={cn(
                      ...
                      multi ? 'bg-memorial-bronze-deep text-memorial-canvas' : 'bg-memorial-sage-deep text-memorial-canvas'
                    )}>
                      {count}×
                    </div>
                  )}
...
function PageCard({
  ...
  onChangeLayout: (pageIdx: number, type: ClassicPageType) => void
...
export function ClassicEditor({
  ...
  const { pages, assignments, assign, orderedImages, changePageLayout, togglePageText, editPageText } =
    useClassicLayout(imageUrls, initialPages)
  ...
  return (
    <div className="h-full overflow-y-auto bg-memorial-canvas">
      ...
      <div className="sticky top-0 z-30 bg-memorial-canvas/95 backdrop-blur-xl border-b border-memorial-line px-6 py-3 flex items-center justify-between">
        <div>
          <p className="text-[10px] uppercase tracking-[0.3em] text-memorial-ink-soft">Classic Album gestalten</p>
            ...
            {multipleImages.length > 0 && unusedImages.length === 0 && (
              <div className="flex items-center gap-1.5 overflow-x-auto max-w-full">
                ...
                {multipleImages.slice(0, 8).map((img) => {
                  const count = usageCount.get(img.previewUrl) ?? 0
                  return (
                    <div key={img.id} className="relative w-10 h-10 rounded-md overflow-hidden flex-shrink-0 ring-2 ring-memorial-bronze">
                      <img src={img.previewUrl} alt="" className="w-full h-full object-cover" />
