...
import { Plus, Check, ChevronDown, AlertCircle, Smartphone } from 'lucide-react'
...
import {
  ...
  GENERIC_PHRASES,
  type SlotPath,
  type VintagePageType,
  type VintagePageConfig,
} from '@/components/album-viewer/vintage/VintagePhotoAlbum'
import { useVintageLayout } from '@/hooks/useVintageLayout'
...
interface Props {
  ...
  initialPages?: VintagePageConfig[]
  onPreview: () => void
  onBack: () => void
  onSave: (payload: { pages: VintagePageConfig[]; assignments: Record<SlotPath, string> }) => void
  onPublish: () => void
  saving: boolean
  publishing: boolean
  isPublished: boolean
...
function buildPageMeta(pages: ReturnType<typeof useVintageLayout>['pages']): PageMeta[] {
...
const LAYOUT_TYPES: { type: VintagePageType; label: string; slots: number }[] = [
  { type: 'single', label: 'Ein Bild',   slots: 1 },
  { type: 'duo',    label: 'Zwei Bilder', slots: 2 },
]

function LayoutWireframe({ type }: { type: VintagePageType }) {
  const base = 'bg-amber-700/30 rounded-[2px]'
  switch (type) {
    case 'single':
      return (
...
function LayoutPickerMini({
  ...
  currentType: VintagePageType
  onChange: (type: VintagePageType) => void
}) {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        isActive
                          ? 'border-amber-700 bg-amber-700/10 shadow-[0_0_0_3px_rgba(133,92,56,0.1)]'
                          : 'border-memorial-line hover:border-amber-600/60 hover:bg-memorial-canvas hover:-translate-y-0.5'
                      )}
                    ...
                    >
                      ...
                      {isActive && (
                        <div className="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-amber-700 flex items-center justify-center">
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    isActive
                      ? 'ring-2 ring-amber-700 shadow-[0_0_0_4px_rgba(133,92,56,0.15)]'
                      : 'ring-memorial-line hover:ring-amber-600/60 hover:-translate-y-0.5'
                  )}
                  title={count === 0 ? 'Noch nicht verwendet' : `${count}× verwendet${multi ? ' (mehrfach)' : ''}`}
                >
                  <img src={img.previewUrl} alt="" className={cn('w-full h-full object-cover', isUsed && !isActive && 'opacity-70')} loading="lazy" />
                  {count > 0 && (
                    <div className={cn(
                      ...
                      multi ? 'bg-amber-700 text-amber-50' : 'bg-green-700 text-green-50'
                    )}>
                      {count}×
                    </div>
                  )}
                  ...
                  {isActive && (
                    <div className="absolute bottom-1.5 right-1.5 w-5 h-5 rounded-full bg-amber-700 flex items-center justify-center ring-2 ring-white">
...
function PageCard({
  ...
  onChangeLayout: (pageIdx: number, type: VintagePageType) => void
          ...
          {canToggleText && (
            <button
              ...
              className={cn(
                ...
                showText
                  ? 'border-amber-700 bg-amber-700/10 text-amber-800'
                  : 'border-memorial-line text-memorial-ink-soft hover:border-amber-600/50'
              )}
            >
              <span className={cn('w-1.5 h-1.5 rounded-full', showText ? 'bg-amber-700' : 'bg-memorial-ink-soft/30')} />
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    active
                      ? 'border-amber-700 bg-amber-700/10 text-amber-800'
                      : 'border-memorial-line text-memorial-ink-soft hover:border-amber-600/50 hover:text-memorial-ink'
                  )}
                >
                  {phrase}
                </button>
          ...
          return (
            <button
              ...
              className={cn(
                ...
                'ring-1 ring-memorial-line hover:ring-amber-600 hover:shadow-lg'
              )}
            >
              {url ? (
                <>
                  <img src={url} alt={s.label} className="w-full h-full object-cover" style={{ filter: 'sepia(0.3)' }} />
...
export function VintageEditor({
  ...
  const { pages, assignments, assign, orderedImages, changePageLayout, togglePageText, editPageText } =
    useVintageLayout(imageUrls, initialPages)
  ...
  return (
    <div className="h-full overflow-y-auto bg-memorial-canvas">
      ...
      <div className="sticky top-0 z-30 bg-memorial-canvas/95 backdrop-blur-xl border-b border-memorial-line px-6 py-3 flex items-center justify-between">
        <div>
          <p className="text-[10px] uppercase tracking-[0.3em] text-memorial-ink-soft">Vintage Album gestalten</p>
                ...
                {multipleImages.slice(0, 8).map((img) => {
                  ...
                  return (
                    <div key={img.id} className="relative w-10 h-10 rounded-md overflow-hidden flex-shrink-0 ring-2 ring-amber-500">
                      <img src={img.previewUrl} alt="" className="w-full h-full object-cover" />
                      <span className="absolute -top-1 -right-1 bg-amber-700 text-white text-[10px] font-bold rounded-full min-w-[16px] h-[16px] flex items-center justify-center px-1">
                        {count}
                      </span>
                    </div>
                  )
