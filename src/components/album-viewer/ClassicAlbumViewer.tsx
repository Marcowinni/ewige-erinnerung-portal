...
import { ClassicPhotoAlbum } from './classic/ClassicPhotoAlbum'
import {
  isEditorPages,
  buildMediaIdUrlMap,
  editorPagesToClassic,
} from '@/lib/editorLayoutConverter'
import { isStoredPages, fromStoredClassicPages } from '@/lib/storedAlbumLayout'
import type { FocalByUrl } from '@/lib/album/focalContext'

interface ClassicAlbumViewerProps {
...
export function ClassicAlbumViewer({
  albumLayout,
  imageSources,
  subjectName,
  dedication,
  dateRange,
}: ClassicAlbumViewerProps) {
  ...
  if (loading && files.length > 0) {
    return (
      <div className="flex items-center justify-center h-full w-full" style={{ background: '#1a0f08' }}>
        <div className="w-10 h-10 border-2 border-dashed rounded-full animate-spin" style={{ borderColor: '#b89c72' }} />
      </div>
    )
  }
