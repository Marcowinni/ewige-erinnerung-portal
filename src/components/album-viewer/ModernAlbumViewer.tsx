...
import { ModernPhotoAlbum } from './modern/ModernPhotoAlbum'
import {
  isEditorPages,
  buildMediaIdUrlMap,
  editorPagesToModern,
} from '@/lib/editorLayoutConverter'
import { isStoredPages, fromStoredModernPages } from '@/lib/storedAlbumLayout'
import type { FocalByUrl } from '@/lib/album/focalContext'

interface ModernAlbumViewerProps {
...
export function ModernAlbumViewer({
  albumLayout,
  imageSources,
  subjectName,
  dedication,
  dateRange,
}: ModernAlbumViewerProps) {
  ...
  if (loading && files.length > 0) {
    return (
      <div className="flex items-center justify-center h-full w-full" style={{ background: 'oklch(0.96 0.012 82)' }}>
        <div className="w-10 h-10 border-2 border-dashed rounded-full animate-spin" style={{ borderColor: 'oklch(0.62 0.06 65)' }} />
      </div>
    )
  }
