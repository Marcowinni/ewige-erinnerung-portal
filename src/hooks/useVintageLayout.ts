...
import type { SlotPath } from '@/components/album-viewer/vintage/VintagePhotoAlbum'

export interface VintageLayoutState {
  pages: VintagePageConfig[]
  assignments: Record<SlotPath, string>
}

function buildAssignments(images: string[]): Record<SlotPath, string> {
  const pages = buildVintagePages(images)
  return assignmentsFromPages(pages)
}

function assignmentsFromPages(pages: VintagePageConfig[]): Record<SlotPath, string> {
  const assignments: Record<SlotPath, string> = {}
  for (let pageIdx = 0; pageIdx < pages.length; pageIdx++) {
    const page = pages[pageIdx]!
    for (const { slotPath, img } of slotsForVintagePage(page, pageIdx)) {
...
export function useVintageLayout(images: string[], initial?: VintagePageConfig[]) {
  const initialPages       = useMemo(
    () => (initial && initial.length > 0 ? initial : buildVintagePages(images)),
    [images, initial]
  )
  ...
  const [pages, setPages]             = useState<VintagePageConfig[]>(initialPages)
  ...
  const changePageLayout = useCallback((pageIdx: number, newType: VintagePageType) => {
    setPages((prevPages) => {
  ...
  const addPage = useCallback((afterIdx: number, type: VintagePageType) => {
    setPages((prevPages) => {
      setAssignments((prevAssignments) => {
